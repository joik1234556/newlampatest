"""VideoCDN scraper – search and iframe/direct link extraction."""
...
from app.config import VIDEOCDN_MIRRORS
...
def _parse_search_results(html: str, base_url: str) -> list[dict]:
    ...
    items: list[dict] = []
    for card in soup.select(".films-item, .short-film, article.item, .movie-item"):
        ...
        items.append({"title": title, "year": year, "poster": poster, "url": url, "source": "videocdn"})
    return items
...
async def search(query: str) -> list[dict]:
    return await try_mirrors(VIDEOCDN_MIRRORS, lambda mirror: search_mirror(mirror, query))
